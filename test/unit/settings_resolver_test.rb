...
class KalvadSlackSettingsResolverTest < RedmineKalvadSlack::TestCase
  ...
  def test_global_webhook_url_when_no_override
    assert_equal 'https://global.test/x', RedmineKalvadSlack::SettingsResolver.webhook_url(@project)
  end

  def test_project_overrides_webhook_url
    KalvadSlackSetting.create!(project_id: @project.id, webhook_url: 'https://proj.test/y')
    assert_equal 'https://proj.test/y', RedmineKalvadSlack::SettingsResolver.webhook_url(@project)
  end

  def test_channel_dash_passes_through
    KalvadSlackSetting.create!(project_id: @project.id, channel: '-')
    assert_equal '-', RedmineKalvadSlack::SettingsResolver.channel(@project)
  end
  ...
  def test_tri_state_force_off_overrides_global_true
    ...
    assert_not RedmineKalvadSlack::SettingsResolver.bool?(@project, :post_issue_created)
  end
  ...
  def test_tri_state_force_on_overrides_global_false
    ...
    assert RedmineKalvadSlack::SettingsResolver.bool?(@project, :post_wiki_updated)
  end
  ...
  def test_inherit_falls_through_to_parent
    ...
    assert_not RedmineKalvadSlack::SettingsResolver.bool?(child, :post_news)
  end
end
