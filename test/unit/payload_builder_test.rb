...
class KalvadSlackPayloadBuilderTest < RedmineKalvadSlack::TestCase
  ...
  def test_issue_created_shape
    issue = Issue.find(1)
    payload = KalvadSlack::PayloadBuilder.issue_created(issue)
    assert_kind_of Hash, payload
    assert payload[:text].include?("##{issue.id}")
    att = payload[:attachments].first
    assert_equal KalvadSlack::Color::CREATED, att[:color]
  ...
  def test_issue_closed_uses_closed_color
    ...
    payload = KalvadSlack::PayloadBuilder.issue_closed(issue, journal)
    assert_equal KalvadSlack::Color::CLOSED, payload[:attachments].first[:color]
  end
  ...
  def test_news_payload
    ...
    payload = KalvadSlack::PayloadBuilder.news_created(news)
    att = payload[:attachments].first
    assert_equal KalvadSlack::Color::NEWS, att[:color]
    assert_equal news.title, att[:title]
  end

  def test_escape_handles_brackets
    assert_equal '&lt;a&gt; &amp; &lt;b&gt;', KalvadSlack::PayloadBuilder.escape('<a> & <b>')
  end
end
